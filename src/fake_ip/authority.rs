    ...
    async fn do_lookup(
        ...
        match record_type {
            RecordType::A | RecordType::AAAA => {
                ...
                let real_ip = match upstream_record.data() {
                    RData::A(A(ip)) => {
                ...
                let record = Record::from_rdata(name.into(), 60, data);
            ...
            RecordType::HTTPS => {
                ...
                let svcb = match upstream_record.data() {
                    RData::HTTPS(HTTPS(svcb)) => svcb,
                    _ => return Ok(ForwardLookup(lookup)),
                };

                let mut svcb_params: Vec<(SvcParamKey, SvcParamValue)> = Vec::new();
                ...
                record.set_ttl(60).set_data(data);
...
impl Authority for FakeAuthority {
    ...
    async fn lookup(
        ...
        match self.do_lookup(name, record_type).await {
